...
export default function App() {
  ...
  // BLINDAGEM MATEMÁTICA: Se o dado for null, ele assume 0 automaticamente.
  const currentTraditionalTotal = editableItems.reduce((acc, item) => 
    acc + (Number(item?.priceTraditional || 0) * Number(item?.qty || 1)), 0);
    
  const currentWholesaleTotal = editableItems.reduce((acc, item) => 
    acc + (Number(item?.priceWholesale || 0) * Number(item?.qty || 1)), 0);
                ...
                <tbody>
                  {editableItems.map((item, index) => (
                    <tr key={index} className="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors">
                      <td className="py-4 pl-2 font-bold">{item?.name || "Sem Nome"}</td>
                      ...
                      <td className="py-4 text-right pr-4 font-mono text-slate-300">
                         {/* Proteção aplicada aqui para impedir que a tela fique em branco */}
                         R$ {Number((item?.priceTraditional || 0) * (item?.qty || 1)).toFixed(2)}
                      </td>
                      <td className="py-4 text-right pr-2 font-mono font-bold text-emerald-400">
                         {/* Proteção aplicada aqui para impedir que a tela fique em branco */}
                         R$ {Number((item?.priceWholesale || 0) * (item?.qty || 1)).toFixed(2)}
                      </td>
                      <td className="py-4 text-center">
