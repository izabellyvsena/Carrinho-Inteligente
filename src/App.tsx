...
export default function App() {
  ...
  // Custom User Profile to scale the niche calculations (Domestic vs Confectionery/Event entrepreneur)
  const [purchaseProfile, setPurchaseProfile] = useState<"domestico" | "confeitaria">("domestico");

  // States for polished user feedback modal
  ...
  // Keep track of source of modifications to keep editor and list completely unified
  const [lastSource, setLastSource] = useState<"text" | "table">("text");

  // Custom manual adjustments state
  const [editableItems, setEditableItems] = useState<Item[]>(DEFAULT_RESULT.items);

  // Double-sided table sync so that items updated, added or deleted automatically update list text
  ...
  // History state with persistent initial seed
  ...
  // Keep editable items synchronized when a new analysis result returns
  useEffect(() => {
    if (result) {
  ...
  // Save favorite markets state to local storage when changed
  useEffect(() => {
    localStorage.setItem("carrinho_inteligente_favorite_markets", JSON.stringify(favoriteMarkets));
  }, [favoriteMarkets]);

  // Handle direct item edits (real-time recalculating on front-end!)
  const handleUpdatePrice = (index: number, field: "priceTraditional" | "priceWholesale", value: number) => {
    const updated = [...editableItems];
    // Garante que se o valor for NaN (inválido) ou menor que zero, ele se torne 0
  ...
  const handleRemoveItem = (index: number) => {
    ...
    // Feedback tátil micro ao remover
  ...
  const handleAddItem = () => {
    ...
    // Feedback tátil micro ao adicionar
    if (typeof navigator !== "undefined" && typeof navigator.vibrate === "function") {
      navigator.vibrate(45);
    }
  };

  // Recalculates totals immediately based on modified table state
  const currentTraditionalTotal = editableItems.reduce((acc, item) => acc + item.priceTraditional, 0);
  const currentWholesaleTotal = editableItems.reduce((acc, item) => acc + item.priceWholesale, 0);
  ...
  // Fetch core address coordinates using native geolocation and reverse-geocoding
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // Use OpenStreetMap free reverse geocoding API with maximum resolution
          const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&zoom=18`);
          if (res.ok) {
            const data = await res.json();
            const address = data.address || {};
            // Extract precise neighborhood, city, and state code for Brazilian locales
          ...
          console.error(err);
          Sentry.captureException(err, { tags: { section: "getGPSLocation_reverse_geocode" } });
       ...
       (err) => {
         console.error(err);
         Sentry.captureException(err, { tags: { section: "getGPSLocation_hardware_or_permission" } });
  ...
  // Calls server-side Gemini API route securely to optimize
  const optimizeShoppingList = async (textToAnalyse: string, locOverride?: string) => {
    ...
    // Nova trava de segurança para textos absurdamente longos
    ...
    // Package payload smartly depending on whether raw text or table edits was last touched
    const payload: any = {
    ...
    try {
      ...
      // Haptic hibrido de feedback tátil para uma experiência nativa polida
      if (typeof navigator !== "undefined" && typeof navigator.vibrate === "function") {
      ...
      Sentry.captureException(err, { 
        tags: { 
          section: "optimizeShoppingList",
          purchaseProfile: purchaseProfile 
        },
        extra: {
          inputTextLength: textToAnalyse.length,
  ...
  const handleDeleteHistoryItem = (id: string, e: MouseEvent) => {
    e.stopPropagation(); // Avoid triggering list restoration
  ...
  const handleRestoreList = (pastList: SavedList) => {
    // Generate raw text representation to populate input
    ...
    // Create custom result for the reconstructed past list
    setResult({
      items: pastList.items,
      suggestions: result.suggestions, // Keep existing smart suggestions
    ...
    return (
      <div className="min-h-screen bg-[#FAF9F6] text-slate-900 font-sans selection:bg-emerald-100 selection:text-emerald-950 p-6 md:p-12 lg:p-16 flex flex-col justify-center items-center relative overflow-hidden">
        ...
        <div className="max-w-4xl w-full relative z-10 my-auto flex flex-col items-center">
          ...
          {/* Key Value Props (Deluxe Bento Grid Layout) */}
          <motion.div 
          ...
          {/* User Location Setup Portal Card */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-slate-900 font-sans selection:bg-emerald-100 selection:text-emerald-950 p-6 md:p-12 lg:p-16 flex flex-col justify-between relative overflow-x-hidden">
      ...
      {/* Decorative Blur Orbs for modern organic depth */}
      ...
      {/* HEADER SECTION - ELITE BENTO HEADER */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-grow relative z-10">
        ...
        {/* LEFT COLUMN: SHOPPING LIST AND SAMPLES (ColSpan 4) */}
        <div className="col-span-1 lg:col-span-4 flex flex-col gap-6">
          
          {/* LOCATION SELECTOR & GPS DISCOVERY */}
          <motion.div 
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            {/* PERFIL DE COMPRA SELECTION FOR NICHE CUSTOMIZATION */}
        ...
        {/* CENTER / RIGHT COLUMN: MAIN TABLE COMPARATIVE & ROBOT FEEDBACK (ColSpan 8) */}
            ...
            <div className="overflow-x-auto min-h-[300px]">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100/80 text-xs">
                  {loading ? (
                    // Premium Skeleton Screens representation of items loading with animated state pulses
                        ...
                        <tr key={index} className="hover:bg-slate-50/70 transition-colors group">
                          ...
                          <td className="py-3 text-right pr-3">
                            <div className="inline-flex items-center gap-1 justify-end">
                              <span className="text-[10px] text-slate-400 font-extrabold font-mono">R$</span>
                              <input
                                type="number"
                                step="0.01"
                                value={item.priceTraditional}
                                onChange={(e) => handleUpdatePrice(index, "priceTraditional", parseFloat(e.target.value) || 0)}
                                className="bg-transparent hover:bg-slate-100/70 focus:bg-white focus:ring-4 focus:ring-slate-150/50 rounded-xl px-1.5 py-1.5 font-mono text-xs md:text-sm text-slate-800 w-16 text-right outline-none font-bold"
                              />
                            </div>
                          </td>
                          <td className="py-3 text-right pr-3 font-black text-emerald-600">
                            <div className="inline-flex items-center gap-1 justify-end">
                              <span className="text-[10px] text-emerald-400 font-mono font-black">R$</span>
                              <input
                                type="number"
                                step="0.01"
                                value={item.priceWholesale}
                                onChange={(e) => handleUpdatePrice(index, "priceWholesale", parseFloat(e.target.value) || 0)}
                                className="bg-transparent hover:bg-emerald-55 focus:bg-white focus:ring-4 focus:ring-emerald-250/35 rounded-xl px-1.5 py-1.5 font-mono text-xs md:text-sm text-emerald-800 font-extrabold w-16 text-right outline-none"
                              />
                            </div>
                          </td>
      ...
      {/* HISTÓRICO COMPLETO & GRÁFICO DE EVOLUÇÃO - NEW BENTO ELEMENT */}
      <motion.section 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        {/* HISTORICAL TIMELINE LISTS - ColSpan 5 */}
        <motion.div 
          whileHover={{ y: -4 }}
        ...
        >
          ...
          <div>
            ...
            {/* DYNAMIC ACCUMULATED SAVINGS CARD */}
        ...
        {/* VISUAL REPORT & METRIC CHARTS - ColSpan 7 */}
        <motion.div 
          whileHover={{ y: -4 }}
        ...
        >
          <div>
            ...
            {/* INTERACTIVE WEEKLY COLUMN CHART */}
            <div className="space-y-4">
              <div className="h-44 flex items-end justify-between gap-3 border-b border-slate-800/80 pb-2.5 pt-4">
                {savedLists.map((list, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Side-by-side comparative bars stacked inside single line */}
                      <div className="w-full flex items-end gap-1.5 h-full max-w-[48px] justify-center">
                        {/* Traditional Bar (Slate Darker) */}
                        <div 
                ...
                {/* ACTIVE LIST IN TABLE (Dynamic column updated instantly on state modification!) */}
            ...
            {/* GENERAL STATS SUMMARY METRIC - CUMULATIVE */}
      ...
      {/* LOWER REAL-TIME STATIC STATS (Trocas Inteligentes, Economia Total, Melhores Mercados) */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mt-8 relative z-10">
        
        {/* BENTO 1: SMART SWAPS (Trocas Inteligentes) - ColSpan 4 */}
        ...
        {/* BENTO 2: VOLUME SUGGESTIONS (Dicas adicionais de Volume) - ColSpan 4 */}
        ...
        {/* BENTO 3: BIG SUMMARY BANNER (Banner de Economia Total) - ColSpan 4 */}
      ...
      {/* REGIONAL RETAILERS MAP INFO (Rodoviária e Atacadão) */}
              ...
              <div className="p-6 relative z-10">
                {!feedbackSubmitted ? (
                  <form onSubmit={(e) => {
                    ...
                    // Save feedback to localStorage so that they feel simulated persistence
